...
export function exportJournalCsv(entries, user) {
  const headerRow = CSV_COLUMNS.map(([label]) => escapeCsvField(label)).join(",");
  const dataRows = entries.map((entry) =>
    CSV_COLUMNS.map(([, accessor]) => escapeCsvField(accessor(entry))).join(","),
  );
...
export function exportJournalPdf(entries, user) {
  const JsPdf = getJsPdf();

  if (!JsPdf) {
    return exportJournalPdfFallback(entries, user);
  }
  ...
  doc.autoTable({
    ...
    didDrawPage: (data) => {
      ...
      const metaParts = [ownerLine, `Generated: ${generatedOn}`, `Trades: ${entries.length}`].filter(Boolean);
...
function exportJournalPdfFallback(entries, user) {
